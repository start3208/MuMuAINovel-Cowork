...
def write_workspace_readme(output_dir: Path, data: dict[str, Any]) -> None:
    ...
    lines = [
        "# MuMu Workspace",
        "",
        "This folder is generated from a MuMuAINovel export JSON file.",
        "",
        f"- project: {project.get('title', 'unknown')}",
        f"- version: {data.get('version', '')}",
        f"- export_time: {data.get('export_time', '')}",
        "",
        "## Counts",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "",
            "1. Use `ls` to inspect the section folders in this workspace.",
            "2. Open `_index.md` inside a section to map short filenames to real titles.",
            "3. Edit the Markdown files you care about.",
            "4. Run `validate` before converting back to JSON.",
            "5. Import the generated JSON back into MuMuAINovel.",
            "",
            "## Navigation",
            "",
            "```powershell",
            ...
            "## Rules",
            "",
            "- Keep section directories and `_index.md` files in place.",
            "- Edit Markdown content freely, including JSON fenced blocks.",
            "- Short filenames like `ch-001-*`, `char-001-*`, `fs-001-*` are intended for easy `ls` usage.",
            "",
            "Edit the Markdown files, run validate, then convert back to JSON for MuMuAINovel import.",
            "",
        ]
    )
